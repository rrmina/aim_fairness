...
def _read_data_file(path, device, train=True, normalize=True):
    ...
    features, labels = data_frame.drop('income', axis=1), data_frame['income']
    ...
    for col in features.columns:
        if (features[col].isnull().sum() > 0):
            ...
            if (features[col].type == np.object):
                categorical_columns += [col]
            else:
                continuous_vars += [col]

    # Transform Categorical Varables into their One-hot encoding
    features = pd.get_dummies(features, columns=categorical_colums, prefix_sep='=')
    ...
    if (train==False):
        features.insert(
            loc = features.clumns.get_loc('native_country=Holand-Netherlands'),
    ...
    a = np.logical_not( pd.Categorical( features[sensitive_attribute] ).codes )             # pd.Categorical().codes converts categorical strings to categorical integer values
